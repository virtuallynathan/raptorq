...
impl SourceBlockDecoder {
    ...
    fn try_pi_decode(
        ...
        let (intermediate_symbols, operations) = match fused_inverse_mul_symbols(
            constraint_matrix,
            hdpc_rows,
            symbols,
        ...
        let intermediate_symbol_mapping = operations
            .as_ref()
            .and_then(|ops| Self::extract_reorder_mapping(ops));
        ...
        for i in 0..self.source_block_symbols as usize {
            if let Some(ref symbol) = self.source_symbols[i] {
                ...
                self.rebuild_source_symbol_into(
                    ...
                    intermediate_symbol_mapping.as_deref(),
                    i as u32,
                    params,
                );
...
mod codec_tests {
    ...
    fn decode_no_hdpc_matches_standard() {
        ...
        for symbol_count in [10, 50, 100, 250, 500] {
            ...
            // Generate source packets + extra repair packets (~10% overhead)
            let overhead_count = (symbol_count as f64 * 0.10).ceil() as u32;
            let repair_packets = encoder.repair_packets(0, overhead_count);
            let source_packets = encoder.source_packets();

            // Feed all source + repair packets (simulating 10% overhead)
            let mut decoder = SourceBlockDecoder::new(1, &config, elements as u64);
    ...
    fn decode_no_hdpc_repair_only() {
        ...
        for symbol_count in [100, 250, 500] {
            ...
            // Only repair packets, with a few extra for safety
            let repair_count = symbol_count as u32 + 4;
