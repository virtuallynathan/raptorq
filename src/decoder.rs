...
impl SourceBlockDecoder {
    ...
    pub fn decode<T: IntoIterator<Item = EncodingPacket>>(
        ...
        // Count total symbols for slab: S + H zeros, received source, padding, repair
        ...
        // Append the extended padding symbols (already zero)
    ...
    fn rebuild_source_symbol_into(
        ...
        let indices = enc_indices(tuple, params.lt_symbols, params.pi_symbols, params.p1);
        let mut first = true;
        for i in indices {
            if first {
                dest.copy_from_slice(mapped(i));
                first = false;
            } else {
                add_assign(dest, mapped(i));
            }
        }
    }
}
