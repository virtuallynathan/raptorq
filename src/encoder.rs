...
pub struct SourceBlockEncoder {
    ...
    // Optional mapping from final intermediate-symbol index -> slab row index.
    intermediate_symbol_mapping: Option<Vec<usize>>,
}
...
impl SourceBlockEncoder {
    ...
    pub fn new(
        ...
        #[cfg(feature = "std")]
        {
            ...
            let (intermediate_symbols, intermediate_symbol_mapping) =
                gen_intermediate_symbols_with_plan(
                    &source_symbols,
            ...
            return SourceBlockEncoder {
                ...
                intermediate_symbol_mapping,
            };
        }

        #[cfg(not(feature = "std"))]
        {
            let (intermediate_symbols, operations) = gen_intermediate_symbols(
                &source_symbols,
                config.symbol_size() as usize,
                SPARSE_MATRIX_THRESHOLD,
            );
            let intermediate_symbol_mapping = operations
                .as_ref()
                .and_then(|ops| extract_reorder_mapping(ops));
    ...
    /// Return the number of source symbols in this block.
    pub fn source_symbols_count(&self) -> u32 {
        self.source_symbols.len() as u32
    }

    /// Return a reference to the bytes of the source symbol at `index`.
    /// No allocation is performed.
    pub fn source_symbol(&self, index: u32) -> &[u8] {
        self.source_symbols[index as usize].as_bytes()
    }

    /// Return the `PayloadId` for the source symbol at `index`.
    pub fn source_symbol_payload_id(&self, index: u32) -> PayloadId {
        PayloadId::new(self.source_block_id, index)
    }

    /// Generate a single repair packet.
    ///
    ...
    ///
    /// `dest.len()` must equal the symbol size of this block.
    /// No heap allocation is performed.
    /// Returns the `PayloadId` for the generated repair symbol.
    pub fn repair_symbol_into(&self, repair_symbol_id: u32, dest: &mut [u8]) -> PayloadId {
        let symbol_size = self.source_symbols[0].as_bytes().len();
        assert_eq!(
            dest.len(),
            symbol_size,
            "dest length must equal symbol size ({})",
            symbol_size,
        );
        let encoding_symbol_id =
            repair_symbol_id + extended_source_block_symbols(self.source_symbols.len() as u32);
...
fn gen_intermediate_symbols(
    ...
    let intermediate_symbols = match (intermediate_symbols, operations.as_ref()) {
        (Some(symbols), Some(ops)) => {
            if let Some(reorder) = extract_reorder_mapping(ops) {
                Some(symbols.gather(&reorder))
            } else {
                Some(symbols)
            }
        }
...
fn extract_reorder_mapping(operation_vector: &[SymbolOps]) -> Option<Vec<usize>> {
    operation_vector.iter().rev().find_map(|op| match op {
        SymbolOps::Reorder { order } => Some(order.clone()),
        _ => None,
    })
}
...
) -> (SymbolSlab, Option<Vec<usize>>) {
    let extended_source_symbols = extended_source_block_symbols(source_block.len() as u32);
    let mut D = create_d(source_block, symbol_size, extended_source_symbols as usize);
    let mut reorder_mapping = None;

    for op in operation_vector {
        match op {
            SymbolOps::Reorder { order } => reorder_mapping = Some(order.clone()),
            _ => perform_op(op, &mut D),
        }
    }
    (D, reorder_mapping)
}
...
fn enc_into(
    ...
    intermediate_symbols: &SymbolSlab,
    intermediate_symbol_mapping: Option<&[usize]>,
    source_tuple: (u32, u32, u32, u32, u32, u32),
) {
    let mapped = |idx: usize| -> &[u8] {
        let row = intermediate_symbol_mapping.map_or(idx, |map| map[idx]);
        intermediate_symbols.get(row)
    };
    ...
    dest.copy_from_slice(mapped(b as usize));
    for _ in 1..d {
        b = (b + a) % w;
        add_assign(dest, mapped(b as usize));
    }
    ...
    add_assign(dest, mapped((w + b1) as usize));
...
mod tests {
    ...
    #[test]
    fn enc_into_matches_enc() {
        let source_symbols = gen_test_symbols();
        let (intermediate_symbols, _) =
            gen_intermediate_symbols(&source_symbols, SYMBOL_SIZE, SPARSE_MATRIX_THRESHOLD);
        let intermediate_symbols = intermediate_symbols.unwrap();

        let lt_symbols = num_lt_symbols(NUM_SYMBOLS);
        let sys_index = systematic_index(NUM_SYMBOLS);
        let p1 = calculate_p1(NUM_SYMBOLS);
        // Test for source symbol indices and some repair indices
        for i in 0..(NUM_SYMBOLS + 20) {
    ...
    #[cfg(not(feature = "python"))]
    #[test]
    fn repair_packet_matches_repair_packets() {
        let data = gen_test_data(1280 * 10);
        let config = ObjectTransmissionInformation::new(data.len() as u64, 1280, 1, 1, 1);
        let encoder = Encoder::new(&data, config);
        let block = &encoder.get_block_encoders()[0];

        let batch = block.repair_packets(0, 5);
        for i in 0..5u32 {
            let single = block.repair_packet(i);
            assert_eq!(batch[i as usize].payload_id(), single.payload_id());
            assert_eq!(batch[i as usize].data(), single.data());
        }
    }

    #[cfg(not(feature = "python"))]
    #[test]
    fn repair_symbol_into_matches_repair_packet() {
        let data = gen_test_data(1280 * 10);
        let config = ObjectTransmissionInformation::new(data.len() as u64, 1280, 1, 1, 1);
        let encoder = Encoder::new(&data, config);
        let block = &encoder.get_block_encoders()[0];

        for repair_id in 0..5u32 {
            let packet = block.repair_packet(repair_id);
            let mut buf = vec![0u8; 1280];
            let payload_id = block.repair_symbol_into(repair_id, &mut buf);
            assert_eq!(*packet.payload_id(), payload_id);
            assert_eq!(packet.data(), &buf[..]);
        }
    }

    #[cfg(not(feature = "python"))]
    #[test]
    fn source_symbol_returns_correct_data() {
        let data = gen_test_data(1280 * 10);
        let config = ObjectTransmissionInformation::new(data.len() as u64, 1280, 1, 1, 1);
        let encoder = Encoder::new(&data, config);
        let block = &encoder.get_block_encoders()[0];

        let packets = block.source_packets();
        for i in 0..block.source_symbols_count() {
            assert_eq!(packets[i as usize].data(), block.source_symbol(i));
            assert_eq!(
                *packets[i as usize].payload_id(),
                block.source_symbol_payload_id(i)
            );
        }
    }
    ...
    #[cfg(not(feature = "python"))]
    #[test]
    #[should_panic(expected = "dest length must equal symbol size")]
    fn repair_symbol_into_wrong_size_panics() {
        let data = gen_test_data(1280 * 10);
        let config = ObjectTransmissionInformation::new(data.len() as u64, 1280, 1, 1, 1);
        let encoder = Encoder::new(&data, config);
        let block = &encoder.get_block_encoders()[0];
        let mut buf = vec![0u8; 999]; // wrong size
        block.repair_symbol_into(0, &mut buf);
    }
}
