...
impl SymbolSlab {
    ...
    pub fn from_symbols(symbols: Vec<Symbol>, symbol_size: usize) -> Self {
        ...
        for symbol in symbols {
            ...
            debug_assert_eq!(bytes.len(), symbol_size);
    ...
    /// Reorder symbols according to `order`: new position `i` gets old symbol `order[i]`.
    pub fn reorder(&mut self, order: &[usize]) {
        debug_assert_eq!(order.len(), self.count);
        let ss = self.symbol_size;
        let mut scratch = vec![0u8; self.count * ss];
        for (new_pos, &old_pos) in order.iter().enumerate() {
            scratch[new_pos * ss..(new_pos + 1) * ss]
